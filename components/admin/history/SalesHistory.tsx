...
export default function SalesHistory() {
  ...
  // Load raw data once
  const { orders } = useFinancialCalculations(); // Hook loads orders automatically
  const cashierHistory = useMemo(() => storageService.loadCashierHistory(), []);

  // Filter Logic (UI specific)
  ...
  // Use hook to calculate metrics based on the FILTERED view
  const { totalRevenue, totalCount, completedOrders } = useFinancialCalculations(filteredOrders);
  ...
  const getChannelInfo = (order: Order) => {
      if (order.isDelivery) return { label: 'DELIVERY', color: 'bg-blue-50 text-blue-600 border-blue-200', icon: Bike };
      if (order.tableNumber) return { label: `MESA ${order.tableNumber}`, color: 'bg-yellow-50 text-yellow-600 border-yellow-200', icon: Layers };
    ...
    <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex flex-col gap-1">
          <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Centro de Controle</h3>
          <div className="flex items-center gap-4 mt-1">
              <button onClick={() => setActiveView('SALES')} className={`text-[10px] font-black uppercase tracking-[0.2em] transition-all border-b-2 pb-1 ${activeView === 'SALES' ? 'text-primary border-primary' : 'text-slate-400 border-transparent hover:text-slate-600'}`}>Todas as Vendas</button>
              <button onClick={() => setActiveView('CASHIER')} className={`text-[10px] font-black uppercase tracking-[0.2em] transition-all border-b-2 pb-1 ${activeView === 'CASHIER' ? 'text-primary border-primary' : 'text-slate-400 border-transparent hover:text-slate-600'}`}>Auditoria de Caixa</button>
          </div>
        </div>
        ...
        {activeView === 'SALES' && (
            <div className="flex items-center gap-3">
                <div className="relative group">
                    ...
                    <input type="text" placeholder="Buscar pedido ou cliente..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 pr-4 py-2 bg-white dark:bg-surface-dark border border-slate-200 dark:border-gray-800 rounded-xl focus:ring-2 focus:ring-primary font-bold text-xs shadow-sm w-full md:w-64 transition-all" />
                </div>
      ...
      {activeView === 'SALES' ? (
        <div className="space-y-6">
            {/* Cards de Resumo - Calculated via Hook */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white dark:bg-surface-dark p-5 rounded-[24px] border border-gray-100 dark:border-gray-800 flex items-center gap-4 shadow-sm">
                    ...
                    <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Receita Visível</p><p className="text-lg font-black tracking-tight font-display text-slate-900 dark:text-white">{formatCurrency(totalRevenue)}</p></div>
                </div>
                <div className="bg-white dark:bg-surface-dark p-5 rounded-[24px] border border-gray-100 dark:border-gray-800 flex items-center gap-4 shadow-sm">
                    ...
                    <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Total Filtro</p><p className="text-lg font-black tracking-tight text-slate-900 dark:text-white">{totalCount} pedidos</p></div>
                </div>
                {/* Outros cards estáticos ou genéricos para o layout */}
                <div className="bg-white dark:bg-surface-dark p-5 rounded-[24px] border border-gray-100 dark:border-gray-800 flex items-center gap-4 shadow-sm">
                ...
                <div className="bg-white dark:bg-surface-dark p-5 rounded-[24px] border border-gray-100 dark:border-gray-800 flex items-center gap-4 shadow-sm">
                    ...
                    <div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Concluídos</p><p className="text-lg font-black tracking-tight text-slate-900 dark:text-white">{completedOrders.length} vendas</p></div>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-surface-dark rounded-[24px] border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
                ...
                {filteredOrders.length === 0 && (
                    <div className="p-12 text-center flex flex-col items-center gap-3 text-slate-300">
                      <FileText className="w-12 h-12 opacity-20" />
                      <p className="font-bold text-xs uppercase tracking-widest">Nenhuma venda encontrada</p>
                    </div>
                )}
            </div>
        </div>
