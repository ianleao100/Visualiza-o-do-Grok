...
import { Clock, User, ArrowUpRight, ArrowDownLeft, MessageSquare, Banknote } from 'lucide-react';
...
export default function CashHistoryTable({ transactions }: CashHistoryTableProps) {
  return (
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        {t.receivedAmount !== undefined && (
                            <span className="text-[9px] font-bold text-slate-400 uppercase italic">
                                Bruto: R$ {t.receivedAmount.toFixed(2)}
                            </span>
                        )}
                    </div>
                  </td>
                  <td className="px-8 py-6 text-right">
                    <div className="flex flex-col items-end">
                        <span className={`text-xs font-bold font-display ${t.changeAmount && t.changeAmount > 0 ? 'text-red-400' : 'text-slate-400'}`}>
                            {t.changeAmount && t.changeAmount > 0 ? `R$ ${t.changeAmount.toFixed(2)}` : '-'} 
                        </span>
                    </div>
                  </td>
                  <td className="px-8 py-6 text-right">
